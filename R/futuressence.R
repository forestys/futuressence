...
futuressence <- function(fgeo = NULL, enreg = F, rep_travail = tempdir(), rep_projet = NULL, rep_data = NULL, rep_clim = NULL, resol_grid = 10, buffer = 100) {
    ...
    for (sp in species) {
        ...
        # on recherche les noms de colonne
        nomcol <- paste0("nc <- c(", ifelse(sp %in% c("fasy", "piab", "pisy", "qupe", "abal", "acca", "acmo", "acop", "acpl", "acps", "algl", "bepe", "cabe", "casa",
        ...
        Predsp_p <- predict(RHS_p, gamdf, type = "response", progress = "text", filename = file.path(dossier_save, paste0(sp, "_8610.tif")), overwrite = TRUE)
        out[[sp]]$present <- Predsp_p
        out[[sp]]$median_present <- median(values(Predsp_p), na.rm = TRUE)
        Predsp_f <- predict(RHS_f, gamdf, type = "response", progress = "text", filename = file.path(dossier_save, paste0(sp, "_4665.tif")), overwrite = TRUE)
